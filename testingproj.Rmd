...
```{r}
railwaypoly_sf = st_read(dsn = "data/railways", layer = "hotosm_idn_jakarta_railways_polygons")
```

```{r}
tmap_mode("view")
tm_shape(railwaypoly_sf) + 
  tm_dots() +
  tm_basemap("OpenStreetMap")
tmap_mode("plot")
```
...
PROX_HEALTHCARE <- dist_dkijkt_terminal_min$Min/1000
```
