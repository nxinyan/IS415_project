...
server <- function(input, output, session) {
        ...
        if (is.null(inFile)) {
            ...
            if ("X" %in% colnames(dkijkt_covid)) {
                dkijkt_covid_sf <- st_as_sf(dkijkt_covid,
                                            coords = c("X", "Y"),
                                            crs=as.numeric(req(input$crsprojectionGwr)))
            } else if ("LAT" %in% colnames(dkijkt_covid)) {
                dkijkt_covid_sf <- st_as_sf(dkijkt_covid,
                                            coords = c("LONG", "LAT"),
                                            crs=4326) %>% 
                    st_transform(crs=as.numeric(req(input$crsprojectionGwr)))
            }
        }
    })
    ...
    bw_gwr <- reactive ({
        ...
        formula_reactive = as.formula(paste(input$dependentGwr, " ~ ", paste(input$independentGwr, collapse= "+")))
        mlr_gwr <- stats::lm(formula=formula_reactive, data=uploaded_data())
