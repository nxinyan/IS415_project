...
packages = c('sf', 'tidyverse', 'readxl', 'tmap', 'raster',  'knitr', 'dplyr','olsrr', 'corrplot', 'ggpubr', 'spdep', 'GWmodel')
...
```{r}
oct21 <- unique_column_data_idkel("data/aspatial/Standar Kelurahan Data Corona (31 October 2021 Pukul 10.00).xlsx")
```
...
unique_column_data_meninggal <- function(data) {
  ...
  extracted_data <- dplyr::select(thedata, `ID_KEL`, `Nama_provinsi`, `nama_kota`, `nama_kecamatan`, `nama_kelurahan`, `POSITIF`, `Meninggal.1`) %>%
...
```{r}
oct21 <- unique_column_data_meninggal("data/aspatial/Standar Kelurahan Data Corona (31 October 2021 Pukul 10.00).xlsx")
```
...
```{r}
write.csv(oct21, "data/covid_oct21.csv", row.names = FALSE)
```
...
Removing the outer islands of Jakarta
...
```{r}
write.csv(dkijkt_ctrd, "data/dkijkt_ctrd_oct21.csv", row.names = FALSE)
```


```{r}
st_write(dkijkt_ctrd, "data/dkijkt_ctrd_oct21_xy.csv", layer_options = "GEOMETRY=AS_XY")
```

```{r}
dkijkt_covid_testing <- read_csv("data/dkijkt_ctrd_oct21.csv")
```

```{r}
dkijkt_covid_testing <- read_csv("data/dkijkt_ctrd_oct21_xy.csv")
...
```{r}
dkijkt_covid_updated_mytest <- read_csv("data/dkijkt_covid_updated.csv")
glimpse(dkijkt_covid_updated_mytest)
```

```{r}
library(corrplot)
corrplot(cor(dkijkt_covid_updated_mytest[, 18:21]), diag = FALSE, order = "AOE",
