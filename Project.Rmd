...
```{r}
# write.csv(dkijkt_covid_updated_xy, "data/dkijkt_covid_updated_xy.csv", row.names = FALSE)
```
...
```{r}
jakarta_covid = read_csv("data/dkijkt_covid_updated.csv") 
jakarta_covid.sf = st_as_sf(jakarta_covid,coords = c("X", "Y"),crs=23845)

glimpse(jakarta_covid.sf)
```

Histogram for all proximity factors

```{r}
prx_attraction = ggplot(data=jakarta_covid.sf, aes(x= `PROX_ATTRACTION`)) +
  geom_histogram(bins=20, color="black", fill="light blue")

prx_worship = ggplot(data=jakarta_covid.sf, aes(x= `PROX_WORSHIP`)) +
  geom_histogram(bins=20, color="black", fill="light blue")

prx_restaurant = ggplot(data=jakarta_covid.sf, aes(x= `PROX_RESTAURANT`)) +
  geom_histogram(bins=20, color="black", fill="light blue")

prx_mall = ggplot(data=jakarta_covid.sf, aes(x= `PROX_MALL`)) +
  geom_histogram(bins=20, color="black", fill="light blue")

prx_supermarket = ggplot(data=jakarta_covid.sf, aes(x= `PROX_SUPERMARKET`)) +
  geom_histogram(bins=20, color="black", fill="light blue")

prx_convenience = ggplot(data=jakarta_covid.sf, aes(x= `PROX_CONVENIENCE`)) +
  geom_histogram(bins=20, color="black", fill="light blue")

prx_kindergartens = ggplot(data=jakarta_covid.sf, aes(x= `PROX_KINDERGARTENS`)) +
  geom_histogram(bins=20, color="black", fill="light blue")

prx_schools = ggplot(data=jakarta_covid.sf, aes(x= `PROX_SCHOOL`)) +
  geom_histogram(bins=20, color="black", fill="light blue")

prx_terminal = ggplot(data=jakarta_covid.sf, aes(x= `PROX_TERMINAL`)) +
  geom_histogram(bins=20, color="black", fill="light blue")

prx_healthcare = ggplot(data=jakarta_covid.sf, aes(x= `PROX_HEALTHCARE`)) +
  geom_histogram(bins=20, color="black", fill="light blue")

prx_railways = ggplot(data=jakarta_covid.sf, aes(x= `PROX_RAILWAYS`)) +
  geom_histogram(bins=20, color="black", fill="light blue")
...
```{r}
# positives = create_map(jakarta_covid.sf, 'POSITIF', "Cumulative Postive Cases as at 31st October 2021")
# positives
```
...
```{r}
# jakarte_covid.slr <- lm(formula=POSITIF ~ JUMLAH_PEN, data = jakarta_covid.sf)
```
...
```{r}
corrplot(cor(jakarte_covid[, col]), diag = FALSE, order = "AOE",
         tl.pos = "td", tl.cex = 0.5, method = "number", type = "upper")
```

```{r}
jakarta_covid.mlr = lm(formula = POSITIF ~ JUMLAH_PEN + PROX_ATTRACTION  + PROX_WORSHIP + PROX_RESTAURANT + PROX_MALL  + PROX_SUPERMARKET + PROX_CONVENIENCE  + PROX_KINDERGARTENS  + PROX_SCHOOL  + PROX_TERMINAL  + PROX_HEALTHCARE + PROX_RAILWAYS, data=jakarta_covid.sf)
summary(jakarta_covid.mlr)
```

```{r}
jakarta_covid.mlr1 = lm(formula = POSITIF ~ JUMLAH_PEN + PROX_RESTAURANT + PROX_TERMINAL, data=jakarta_covid.sf)
# summary(dkijkt_covid_updated_xy.mlr1)
ols_regress(jakarta_covid.mlr1)
```

```{r}
ols_vif_tol(jakarta_covid.mlr1)
```

```{r}
ols_plot_resid_fit(jakarta_covid.mlr1)
```

```{r}
ols_plot_resid_hist(jakarta_covid.mlr1)
```

```{r}
ols_test_normality(jakarta_covid.mlr1)
```

```{r}
mlr.output <- as.data.frame(jakarta_covid.mlr1$residuals)
```

```{r}
jakarta_covid.res.sf <- cbind(jakarta_covid.sf,  jakarta_covid.mlr1$residuals) %>%
rename(`MLR_RES` = `jakarta_covid.mlr1.residuals`)
```

```{r}
jakarta_covid.sp <- as_Spatial(jakarta_covid.res.sf)
jakarta_covid.sp
```
...
```{r}
nb <- dnearneigh(coordinates(jakarta_covid.sp), 0, 3500, longlat = FALSE)
summary(nb)
```
...
```{r}
lm.morantest(jakarta_covid.mlr1, nb_lw)
```

```{r}
bw.fixed <- bw.gwr(formula = POSITIF ~ JUMLAH_PEN + PROX_RESTAURANT + PROX_TERMINAL, data=jakarta_covid.sp, approach="CV", kernel="gaussian", adaptive=FALSE, longlat=FALSE)
```

```{r}
gwr.fixed <- gwr.basic(formula = POSITIF ~ JUMLAH_PEN + PROX_RESTAURANT + PROX_TERMINAL, data=jakarta_covid.sp, bw=bw.fixed, kernel = 'gaussian', longlat = FALSE)
```
...
```{r}
bw.adaptive <- bw.gwr(formula = POSITIF ~ JUMLAH_PEN + PROX_RESTAURANT + PROX_TERMINAL, data=jakarta_covid.sp, approach="CV", kernel="gaussian",
adaptive=TRUE, longlat=FALSE)
```

```{r}
gwr.adaptive <- gwr.basic(formula = POSITIF ~ JUMLAH_PEN + PROX_RESTAURANT + PROX_TERMINAL, data=jakarta_covid.sp, bw=bw.adaptive, kernel = 'gaussian', adaptive=TRUE, longlat = FALSE)
```
...
```{r}
jakarta_covid.adaptive <- st_as_sf(gwr.adaptive$SDF) %>%
  st_transform(crs=23845)
```

```{r}
jakarta_covid.adaptive.svy21 <- st_transform(jakarta_covid.adaptive, 23845)
jakarta_covid.adaptive.svy21  
```

```{r}
gwr.adaptive.output <- as.data.frame(gwr.adaptive$SDF)
jakarta_covid.adaptive <- cbind(jakarta_covid.res.sf, as.matrix(gwr.adaptive.output))
```

```{r}
glimpse(jakarta_covid.adaptive)
```
...
tm_shape(jakarta_covid.adaptive) +  
